	...
	*/
	...
	$(document).ready(function() {
		...
		var mygrid = jQuery("#list2").jqGrid({ 
			url:'dashboard/ajaxmylogs/?track=1', 
			datatype: "json", 
			height: '100%',
			/*height: '100%',*/
			width: setwidth-5, 
			mtype: 'GET',
			colNames:['','No. of Comments', 'Issue ID','Status','IMEI', 'Contact No', 'Customer Name', 'Interaction / Case No', 'Issue Description', 'MIN', 'Channel','Organization','Email Address','Date Opened','Date Updated','Date Closed', 'Callback', 'Callback Date/Time', 'Escalated'], 
			colModel:[ {name:'options',index:'options', width:50, search:false, viewable:false},
					   ...
					   {name:'status_id',index:'status_id', width:80, search:false, stype:'select', searchoptions:{sopt:['eq'], dataUrl:"dashboard/getstatusdropdown/"}}, 
