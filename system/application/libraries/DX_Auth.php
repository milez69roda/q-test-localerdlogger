...
class DX_Auth
{
	...
	function _init()
	{
		...
		$this->_session_prefix = $this->ci->config->item('session_data_prefix');
	...
	*/
	...
	function _get_center($center_id){
			
		$this->ci->load->model('centers', 'center');
		
		$center = $this->ci->center->_get_center_by($center_id);
	...
	function autologin()
	{
		...
		if ($auto = $this->ci->input->cookie($this->ci->config->item('DX_autologin_cookie_name')) AND ! $this->ci->session->userdata($this->_session_prefix.'logged_in'))
		{
			// Extract data
	...
	function _set_session($data)
	{
		...
		$center_data = $this->_get_center($data->center_id);
	
		// Set session data array -- ADDED NEW CUSTOM PREFIX FOR SESSION
		$user = array(						
			'user_id'						=> $data->id,
			'username'						=> $data->username,
			'firstname'						=> urldecode($data->fname),
			'lastname'						=> urldecode($data->lname),
			'last_login'					=> $data->last_login,			
			'role_id'						=> $data->role_id,			
			'center_id'						=> $data->center_id,																		
			'center_acronym'				=> $center_data->center_acronym,																		
			'dept_id'						=> $data->dept_id,																		
			'role_name'						=> urldecode($role_data['role_name']),
	...
	*/
	function get_permission_value($key, $check_parent = TRUE)
	{
		...
		$permission = $this->ci->session->userdata($this->_session_prefix.'permission');
		...
		else
		{
			if ($check_parent)
			{
				...
				$parent_permissions = $this->ci->session->userdata($this->_session_prefix.'parent_permissions');
	...
	*/
	function get_permissions_value($key, $array_key = 'default')
	{
		...
		$role_id = $this->ci->session->userdata($this->_session_prefix.'role_id');
		$role_name = $this->ci->session->userdata($this->_session_prefix.'role_name');
		
		$parent_roles_id = $this->ci->session->userdata($this->_session_prefix.'parent_roles_id');
		$parent_roles_name = $this->ci->session->userdata($this->_session_prefix.'parent_roles_name');
	...
	{
		return $this->ci->session->userdata($this->_session_prefix.'user_id');
	}

	// Get username string
	function get_username()
	{
		return $this->ci->session->userdata($this->_session_prefix.'username');
	}
	
	// Get user role id
	function get_role_id()
	{
		return $this->ci->session->userdata($this->_session_prefix.'role_id');
	}
	
	// Get user role name
	function get_role_name()
	{
		return $this->ci->session->userdata($this->_session_prefix.'role_name');
	}
	
	// Check is user is has admin privilege
	function is_admin()
	{
		return strtolower($this->ci->session->userdata($this->_session_prefix.'role_name')) == 'admin';
	}
	...
	function is_role($roles = array(), $use_role_name = TRUE, $check_parent = TRUE)
	{
		...
		if ($check_parent)
		{
			...
			if ($use_role_name)
			{
				$check_array = $this->ci->session->userdata($this->_session_prefix.'parent_roles_name');
			}
			else
			{
				$check_array = $this->ci->session->userdata($this->_session_prefix.'parent_roles_id');
			}
		}
		...
		if ($use_role_name)
		{
			array_push($check_array, $this->ci->session->userdata($this->_session_prefix.'role_name'));
		}
		else
		{
			array_push($check_array, $this->ci->session->userdata($this->_session_prefix.'role_id'));
		}
	...
	{
		return $this->ci->session->userdata($this->_session_prefix.'logged_in');
	}
	...
	function login($login, $password, $remember = TRUE)
	{
		...
		if ( ! empty($login) AND ! empty($password))
		{
			...
			if ($query = $this->ci->users->$get_user_function($login) AND $query->num_rows() == 1)
			{
				...
				{					
					//$password = $this->_encode($password); EDITED BY: Joey
					$password = $password;
					$stored_hash = $row->password;

					// Is password matched with hash in database ?
					//if (crypt($password, $stored_hash) === $stored_hash) EDITED BY: Joey
	...
	{
		// Trigger event
		$this->ci->dx_auth_event->user_logging_out($this->ci->session->userdata($this->_session_prefix.'user_id'));
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata($this->_session_prefix.'user_id')) AND $query->num_rows() > 0)
		{
			...
			if (crypt($pass, $row->password) === $row->password)
			{
				...
				$this->ci->users->change_password($this->ci->session->userdata($this->_session_prefix.'user_id'), $new_pass);
				
				// Trigger event
				$this->ci->dx_auth_event->user_changed_password($this->ci->session->userdata($this->_session_prefix.'user_id'), $new_pass);
	...
	function cancel_account($password)
	{
		...
		if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata($this->_session_prefix.'user_id')) AND $query->num_rows() > 0)
		{
			...
			{
				// Trigger event
				$this->ci->dx_auth_event->user_canceling_account($this->ci->session->userdata($this->_session_prefix.'user_id'));

				// Delete user
				$result = $this->ci->users->delete_user($this->ci->session->userdata($this->_session_prefix.'user_id'));
