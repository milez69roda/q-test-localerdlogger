	...
	$(document).ready(function() {
		...
		var mygrid = jQuery("#list2").jqGrid({ 
			...
			colModel:[ {name:'options',index:'options', width:55, search:false, viewable:false},
					   {name:'lname',index:'lname', width:130, editable:false, search:true, 
							searchoptions:{
					   ...
					   {name:'fname',index:'fname', width:80, hidden:true, editable:true, editrules:{edithidden:true, required:true}, viewable:false}, 
					   {name:'lname1',index:'lname1', width:80, hidden:true, editable:true, editrules:{edithidden:true, required:true}, viewable:false}, 
					   {name:'username',index:'username', width:80, editable:false}, 
					   {name:'password',index:'password', width:90, editable:true, search:true, editrules:{required:true}}, 						  
					   {name:'roles_id',index:'roles_id', width:90, editable:false, hidden:true, search:true}, 						  
					   {name:'center_id',index:'center_id', width:120, editable:false, viewable:false, search:false},
					   {name:'dept_id',index:'dept_id', width:120, editable:false, edittype:'select', editoptions:{dataUrl:'dashboard/getDeptdropdown/'}, search:true, stype:'select', searchoptions:{sopt:['eq'], dataUrl:"dashboard/getDeptdropdown/"}}, 		
					   {name:'supervisor',index:'supervisor', width:130, editable:false, hidden:true, search:true, 
							searchoptions:{
								dataInit:function(el){																									
									$(el).autocomplete({source:"dashboard/ajaxgetUsers/?track=1"});																																																									
								}
