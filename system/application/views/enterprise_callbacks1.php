	...
	*/
	...
	$(document).ready(function() {
		...
		var mygrid = jQuery("#list2").jqGrid({ 
			url:'dashboard/ajaxenterprisecallbacks/?track=1', 
			datatype: "json", 
			height: '100%',
			/*height: '100%',*/
			width: setwidth-5, 
			mtype: 'GET',
			colNames:['','No. of Comments','ID','Status',  'Callback Date/Time','Center', 'Supervisor','Representative', 'Avaya', 'IMEI', 'Contact No', 'Customer Name', 'Case No', 'Issue type', 'Issue Description',  'Callback', 'MIN', 'Channel','Organization','Email Address', 'Escalated'], 
			colModel:[ {name:'options',index:'options', width:60, editable:false, search:false, viewable:false}, 	
					   ...
					   {name:'callback_date',index:'callback_date', width:150, 
							searchoptions:{
...
/* 	function deletelog(gr){
	...
	/* function getSup1(a){
		...
		var ret = $.ajax({
				type:'POST',
				url:'dashboard/getSupervisorDropdown1/'+a,				
				DataType:'jsonp',
				success: function(data){
				}
			}).responseText;
