...
<style type="text/css">
    ...
    <div id="content">
		...
		<div id="main_content">
			...
			<div id="xdownload" style="color: red; background-color:yellow; font-weight:bold; display:none">
				Please wait, while the file is being generated...
			</div>
	...
	*/
	...
	$(document).ready(function() {
		...
		var mygrid = jQuery("#list2").jqGrid({ 
			url:'dashboard/ajaxenterpriseviewlog/?track=1', 
			datatype: "json", 
			height: '100%',
			/*height: '100%',*/
			width: setwidth-5, 
			mtype: 'GET',
			colNames:['', 'No. of Comments', 'Issue ID','Center', 'Supervisor', 'Representative', 'Avaya', 'Status','IMEI', 'Contact No', 'Customer Name', 'Interaction/Case No', 'Issue Type', 'Issue Description',  'MIN', 'Channel','Organization','Email Address', 'Date Opened','Date Updated','Date Closed', 'Callback', 'Callback Date/Time'], 
			colModel:[ {name:'options',index:'options', width:60, search:false, viewable:false},
					   ...
					   {name:'center_id',index:'center_id', width:120, search:true, stype:'select', 
								searchoptions:{	sopt:['eq'], 
												dataUrl:"dashboard/getcenterdropdown/",
												dataEvents:[{
																type:'change',
																fn:function(e){
																	...
																	$.ajax({
																			type:'POST',
																			url:'dashboard/getSupervisorDropdown1/'+e.target.value,	
																			data: 'center='+e.target.value,																				
																			success: function(responseText){
																				jQuery('#gs_supervisor_id').empty().append(responseText);
																			}
																		}); 															 
					   ...
					   {name:'supervisor_id',index:'supervisor_id', width:150, search:true, stype:'select', searchoptions:{sopt:['eq'], dataUrl:"dashboard/getSupervisorDropdown/"}}, 
					   {name:'lname',index:'lname', width:120, search:true, 
							searchoptions:{
								dataInit:function(el){
									jQuery(el).autocomplete({source:"dashboard/ajaxgetReps/?track=1"});	
								}
							}
					   }, 	
					   {name:'avaya',index:'avaya', width:60, search:true},
					   {name:'status_id',index:'status_id', width:80, search:true, stype:'select', searchoptions:{sopt:['eq'], dataUrl:"dashboard/getstatusdropdown1/"}}, 
					   {name:'imei',index:'imei', width:100, search:true}, 
					   {name:'phone_no',index:'phone_no', width:100, search:true}, 						   
					   ...
					   {name:'date_opened',index:'date_opened', width:105, editable:false, search:false, 
							searchoptions:{
								dataInit:function(el){
					   ...
					   {name:'date_updated',index:'date_updated', width:105, editable:false, search:false, 
							searchoptions:{
								dataInit:function(el){
									$(el).datepicker({dateFormat:'yy-mm-dd'});
								}
					   ...
					   {name:'date_closed',index:'date_closed', width:105, editable:false, search:false, 
							searchoptions:{
								dataInit:function(el){
									$(el).datepicker({dateFormat:'yy-mm-dd'});
								}
					   ...
					   {name:'callback_date',index:'callback_date', width:80, 
							searchoptions:{
								dataInit:function(el){
									$(el).datepicker({dateFormat:'yy-mm-dd'});
								}
					   ...
					   {name:'comments',index:'comments', width:200, search:false} 	 */
			], 
			gridComplete: function(){ 
				...
				for(var i=0;i < ids.length;i++){ 
					...
					be = '<a href="javascript:void(0)" onclick="deletelog('+cl+');" class="alink"> Del </a><br/>';									
					jQuery("#list2").jqGrid('setRowData',ids[i],{options:ae+be});  */
		...
		//resizing grid
		jQuery("#list2").jqGrid('gridResize',{minWidth:350,maxWidth:1024,minHeight:300, maxHeight:900});
		...
		jQuery("#list2").jqGrid('navButtonAdd',"#pager2",{caption:"Export to Excel",title:"Export to Excel",buttonicon :'ui-icon-save', 
														onClickButton:function(){ 
															...
															$.ajax({
																type: "GET",
																url: 'dashboard/ajaxExportActionEnterprise/?track=1&tablType=callback&'+str,
	...
	function popCom(x){
		//alert(x);
		$('<div> this is it. </div>').dialog('open');
	}	
